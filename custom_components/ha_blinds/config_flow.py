...
class HaBlindsOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_thresholds(self, user_input: dict[str, Any] | None = None):
        ...
        schema_dict = {
            ...
            vol.Required(
                CONF_DAYTIME_OPEN_POSITION_SUMMER,
                default=_coerce_int_default(defaults.get(CONF_DAYTIME_OPEN_POSITION_SUMMER), int(DEFAULTS[CONF_DAYTIME_OPEN_POSITION_SUMMER])),
            ): vol.All(vol.Coerce(int), vol.Range(min=50, max=100)),
            vol.Required(
                CONF_DAYTIME_OPEN_POSITION_WINTER,
                default=_coerce_int_default(defaults.get(CONF_DAYTIME_OPEN_POSITION_WINTER), int(DEFAULTS[CONF_DAYTIME_OPEN_POSITION_WINTER])),
            ): vol.All(vol.Coerce(int), vol.Range(min=50, max=100)),
            vol.Required(
                CONF_LUX_LOW_THRESHOLD,
